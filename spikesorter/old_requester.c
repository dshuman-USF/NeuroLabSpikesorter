...
run_job (void)
{
  ...
  if (jpid) {
    ...
    if ((fd = open ("nohup.out", O_WRONLY|O_CREAT|O_TRUNC, 0744)) != 1) PERR ("%d", fd);
...
check_exe (pid_t pid, char *path)
{
  ...
  len = strlen (path);
  if (len + 1 > bufsize)
    buf = realloc (buf, bufsize = len + 2);

  if (asprintf (&exe_name, "/proc/%d/exe", pid) == -1) exit (1);
  linklen = readlink (exe_name, buf, len + 1);
  free (exe_name);
  if (linklen < 0)
    return 0;
  buf[linklen] = 0;
...
{
  char *nohup_path;
  char *nohup_name = "nohup.out";
  char *file[] = {"/bin/bash", "/dev/null", "", "", "/var/local/requests/lock", 0};
  enum {EXE, FD0, FD1, FD2, FCNT};
  pid_t pid;

  if (FCNT >= sizeof file / sizeof file[0]) DIE("");
  
  nohup_path = canonicalize_file_name (nohup_name);
  if (nohup_path == NULL) {
    if (errno == ENOENT)
      return 0;
    else
      PDIE ("Can't canonicalize %s", nohup_name);
  }
...
static void
print_status (char *job, char *filename)
{
...
int
old_requester (int argc, char **argv)
{
  DIR *rqdir;
  FILE *rqfile;
  char *rqdirname = "/var/local/requests";
  struct dirent *de;
  char *my_rq_filename;
  long long my_request_time;
  int fd;
  char *orig_dir;
  char *lockfile_name = "/var/local/requests/lock";
  pid_t jpid;
  bool print_start_status = false;

  if (argc == 4 && strcmp (argv[1], "status") == 0) {
    print_status (argv[2], argv[3]);
    return 0;
  }

  if (argc != 2) {
    printf ("usage: %s experiment_name\n", argv[0]);
    return 0;
  }
  if (asprintf (&my_rq_filename, "%s/%s", rqdirname, argv[1]) == -1) exit (1);
  ...
  fprintf (rqfile, "%16lld %d %lu %s\n", my_request_time, getpid (), get_starttime (getpid ()), argv[1]);
  fclose (rqfile);
  ...
  while (flock (fd, LOCK_EX) == 0) {
    ...
    while (errno = 0, (de = readdir (rqdir)) != NULL) {
      ...
      if (S_ISREG (s.st_mode)) {
        ...
        if (strcmp (de->d_name, argv[1]) == 0 || strcmp (de->d_name, "lock") == 0)
          continue;
        //        rdtscll (time0);
